...
Correlation depends on the `events.list` query and on the extended properties surviving. If someone strips a property in the Google UI, that event becomes an orphan: it is deleted and recreated rather than updated. An event that has lost `publishedBy` itself is worse than an orphan — no listing returns it, so it is left where it is and its Block published alongside it.
